...
long unsigned int Input::joystickSensitivity;
...
Input::Input(const sf::Keyboard::Key &newKey) : typeInput(TypeInput::KEYBOARD),
                                                key(newKey)
{
}

Input::Input(const sf::Mouse::Button &newButton) : typeInput(TypeInput::MOUSE),
                                                   button(newButton)
{
}

Input::Input(const sf::Joystick::Axis &newAxis, unsigned int newIdJoystick, bool newSignAxis) : typeInput(TypeInput::JOYSTICK_AXIS),
                                                                                                axis(newAxis),
                                                                                                idJoystick(newIdJoystick),
                                                                                                signAxis(newSignAxis)
{
}

Input::Input(unsigned int newButtonJoyStick, unsigned int newIdJoystick) : typeInput(TypeInput::JOYSTICK),
                                                                           buttonJoystick(newButtonJoyStick),
                                                                           idJoystick(newIdJoystick)
{
}

Input::Input(const Input &input) noexcept : typeInput(input.typeInput),
                                            key(input.key),
                                            button(input.button),
                                            buttonJoystick(input.buttonJoystick)
{
}

Input::Input(const std::string_view &str) : typeInput(),
                                            key(),
                                            button(),
                                            axis(),
                                            buttonJoystick(),
    ...
    else /* if (str.size() > 16)*/
    {
        for (unsigned int i = 0u; i < sf::Joystick::Count; i++)
        {
            ...
            else
            {
                ...
                {
                    if (str == "JoystickButton" + std::to_string(i) + "-" + std::to_string(j))
                    {
                        buttonJoystick = j;
                        idJoystick = i;
...
std::string Input::to_string() const
{
    ...
    {
        res = "JoystickButton" + std::to_string(idJoystick) + "-" + std::to_string(buttonJoystick);
        break;
    }
    case TypeInput::ERROR_TYPE_INPUT:
    {
        debugLog("error type input")
        break;
    }
    default:
    {
...
bool Input::isPressed() const
{
    ...
    case TypeInput::JOYSTICK_AXIS:
    {
        res = sf::Joystick::isConnected(idJoystick) && sf::Joystick::hasAxis(idJoystick, axis) && (signAxis ? (sf::Joystick::getAxisPosition(idJoystick, axis) > static_cast<float>(joystickSensitivity)) : (sf::Joystick::getAxisPosition(idJoystick, axis) < -static_cast<float>(joystickSensitivity)));
        break;
    }
    case TypeInput::JOYSTICK:
    {
        res = sf::Joystick::isConnected(idJoystick) && sf::Joystick::isButtonPressed(idJoystick, buttonJoystick);
        break;
    }
    case TypeInput::ERROR_TYPE_INPUT:
    {
...
Input::TypeInput Input::getTypeInput() const
{
    return typeInput;
}
...
sf::Joystick::Axis Input::getAxis() const
{
    return axis;
}

bool Input::getSignAxis() const
{
    return signAxis;
}

sf::Mouse::Button Input::getButton() const
{
    return button;
}

unsigned int Input::getButtonJoyStick() const
{
    return buttonJoystick;
}

unsigned int Input::getIdJoystick() const
{
    return idJoystick;
}

long unsigned int Input::getJoystickSensitivity()
{
    return joystickSensitivity;
}

void Input::setJoystickSensitivity(long unsigned int newJoystickSensitivity)
{
...
{
    joystickSensitivity = 50; // Useful ?
}

bool operator==(const Input &left, const Input &right)
{
    bool res = false;
    if (left.getTypeInput() != right.getTypeInput())
    {
        res = false;
    }
    else
    {
        switch (left.getTypeInput())
        {
            case Input::TypeInput::KEYBOARD:
            {
                res = left.getKey() == right.getKey();
                break;
            }
            case Input::TypeInput::MOUSE:
            {
                res = left.getButton() == right.getButton();
                break;
            }
            case Input::TypeInput::JOYSTICK_AXIS:
            {
                res = (left.getAxis() == right.getAxis() && left.getSignAxis() == right.getSignAxis() && left.getIdJoystick() == right.getIdJoystick());
                break;
            }
            case Input::TypeInput::JOYSTICK:
            {
                res = (left.getButtonJoyStick() == right.getButtonJoyStick() && left.getIdJoystick() == right.getIdJoystick());
                break;
            }
            case Input::TypeInput::ERROR_TYPE_INPUT:
            {
                debugLog("error type input")
                break;
            }
...
{
    return static_cast<long unsigned int>(inputFl1) | static_cast<long unsigned int>(inputFl2);
}

long unsigned int operator|(long unsigned int inputFl1, const Input::InputFl &inputFl2)
{
    return inputFl1 | static_cast<long unsigned int>(inputFl2);
}
